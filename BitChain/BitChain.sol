...
contract BitChain is ERC20, Ownable, ReentrancyGuard {
    ...
    event UpdateBuyTax(uint256 marketingTaxBuy, uint256 teamTaxBuy);
    event UpdateSellTax(uint256 marketingTaxSell, uint256 teamTaxSell);
    event UpdateTransferTax(
        uint256 marketingTaxTransfer,
        uint256 teamTaxTransfer
    );
    ...
    function SetBuyTax(
        ...
        require(
            marketingTaxBuy != _marketingTaxBuy || teamTaxBuy != _teamTaxBuy,
            "Buy Tax already on that value"
        );
        require(
            _marketingTaxBuy + _teamTaxBuy + marketingTaxSell + teamTaxSell <=
                2_500,
        ...
        emit UpdateBuyTax(_marketingTaxBuy, _teamTaxBuy);
    }

    function SetSellTax(
        uint256 _marketingTaxSell,
        uint256 _teamTaxSell
    ) external onlyOwner {
        require(
        ...
        require(
            marketingTaxBuy + teamTaxBuy + _marketingTaxSell + _teamTaxSell <=
                2_500,
            "Buy Tax and Sell Tax cannot be more than 25%"
        );
        ...
        emit UpdateSellTax(_marketingTaxSell, _teamTaxSell);
    }

    function SetTransferTax(
        uint256 _marketingTaxTransfer,
        uint256 _teamTaxTransfer
    ) external onlyOwner {
        require(
            marketingTaxTransfer != _marketingTaxTransfer ||
                teamTaxTransfer != _teamTaxTransfer,
        ...
        emit UpdateTransferTax(_marketingTaxTransfer, _teamTaxTransfer);
    }
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                ...
                tempMarketingAmount =
                    ...
                    denominator;
                tempTeamAmount = (amount * teamTaxTransfer) / denominator;
            }
