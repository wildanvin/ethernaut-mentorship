...
contract PrePOMarketFactory is IPrePOMarketFactory, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize() public initializer { OwnableUpgradeable.__Ownable_init(); } //@audit-issue should this be public? I guess this is to prevent that issue that we learned about in Ethernaut about non initialized proxies.
  ...
  function createMarket(string memory _tokenNameSuffix,
    ...
    require(validCollateral[_collateral], "Invalid collateral"); //@audit-info can you provide invalid collateral?
    ...
    deployedMarkets[_salt] = address(_newMarket); //@audit-info can you provide two same _salt?
